...
from reverse_string_agent import ReverseStringTask, load_tasks, prompt_template_baseline
...
def vllm_server(
    ...
    Args:
        model_path: The model path to serve. It can either be a local path (downloaded from Hugging Face or local filesystem) or a Hugging Face model ID.
    ...
    try:
        ### CODE HERE will run the server in the background
        vllm_serve_args = [
            ...
            "--port",
            str(port),
        ]  # default arguments for vLLM server. Is a list of strings. because the linux command line will be used to run the server.
        
        # check if quantization is provided and add it to the arguments
        if quantization is not None:
            vllm_serve_args.append("--quantization")
            vllm_serve_args.append(quantization)

        # check if auto tool choice is enabled and add it to the arguments
        if auto_tool_choice:
            vllm_serve_args.append("--enable-auto-tool-choice")

        # check if tool call parser is provided and add it to the arguments
        ...
        while True:
            try:
                if client.get(url).status_code == 200:  # check if the server is running and healthy
                    break #break the loop cuase server is healthy and return the url
            except Exception:
                # wait for server to be ready within the timeout else raise an error
                if proc is not None:
                    result = proc.poll() 
                    if result is not None:
                        raise RuntimeError(f"Server exited unexpectedly with code {result}.") from None
                time.sleep(0.5)
                if time.time() - start > startup_timeout:
                    raise RuntimeError(f"Server failed to start in {startup_timeout} seconds.") from None

        yield f"http://localhost:{port}/v1" # return the url of the server
    finally:
        ### CODE HERE will terminate the server
        
        if proc is not None:
            proc.terminate() # terminate the server
            try:
                proc.wait(terminate_timeout) # wait for the server to terminate gracefully
            except subprocess.TimeoutExpired:
                proc.kill() # kill the server if it doesn't terminate gracefully
